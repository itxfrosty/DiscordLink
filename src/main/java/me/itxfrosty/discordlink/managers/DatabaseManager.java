...
public class DatabaseManager {
	 ...
	 */
	public void connect() {
		String sql = "CREATE TABLE IF NOT EXISTS linked_users (player_uuid VARCHAR(100), discord_id VARCHAR(100));";
		try {
	 ...
	 */
	public void insert(UUID player, long discordID) {
	...
	/**
	 * Get's Discord ID using Player's UUID.
	 *
	 * @param player Player.
	 * @return Player's Discord ID.
	 */
	public Long getDiscordID(UUID player) {
		String SQL = String.format("SELECT * FROM linked_users WHERE player_uuid = \"%h\";", player);
		this.refreshConnection();
		try {
			PreparedStatement ps = connection.prepareStatement(SQL);
